...
for audio_file in os.listdir(CONVERTED_AUDIO_DIR):
    ...
    curl_command = ['curl', '-F', 'audio=@' + os.path.join(CONVERTED_AUDIO_DIR, audio_file), '-F', 'transcript=@' + os.path.join(CONVERTED_TRANSCRIPT_DIR, filename + '.txt'), 'http://localhost:' + DOCKER_PORT + '/transcriptions?async=false']

    print('calling command: {}'.format(curl_command))
